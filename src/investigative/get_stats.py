...
# expects src_sentences and trg_sentences to be lists of str(sentence)'s
def get_src_trg_pairs(src_sentences, trg_sentences, start=1, num=1):
    # use 1-based indexing so that matches UNIX line numbering of files
    pairs = []
    for i in range(start-1, start+num-1):
        pairs.append((src_sentences[i], "#####", trg_sentences[i]))
    return pairs
