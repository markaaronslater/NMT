...
def train(hyperparams, train_batches, dev_batches, references,
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        bleu, preds_time = predict(model, dev_batches, references, idx_to_trg_word, checkpoint_path, epoch, inference_alg="greedy_search", write=True)
        model.train()
        report_stats(epoch, epoch_loss, epoch_time, preds_time, bleu, checkpoint_path)
...
def report_stats(ep, ep_loss, ep_time, preds_time, bleu, checkpoint_path):
...
def initialize_optimizer(model, hyperparams):
    ...
    lr = hyperparams["optimization_alg"]
...
def store_checkpoint(self, model, optimizer, epoch, epoch_loss, bleu, prev_bleu,
